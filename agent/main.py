...
from agent.tools.database import database_query
from agent.tools.voicemail import detect_voicemail
from agent.tools.knowledge import query_ipc_knowledge, get_knowledge_base
...
class IPCAssistant(Agent):
        ...
        async def capture_transcripts():
            try:
                ...
                if hasattr(self, "transcription_node") and self.transcription_node:
                    ...
                    logger.info(
                        "✅ Transcription node available - transcripts will be captured"
                    )
                            ...
                            if transcription.segments:
                                for segment in transcription.segments:
                                    ...
                                    if text:
                                        ...
                                        is_user = (
                                            segment.participant_identity != "agent"
                                        )
                    ...
                    if hasattr(self.transcription_node, "on"):
                        self.transcription_node.on(
                            "transcription_updated", on_transcription_update
                        )
    ...
    async def on_enter(self, ctx: llm.ChatContext) -> None:
        ...
        if self.is_phone_call and not self._has_greeted:
            ...
            try:
                ...
                logger.info(
                    f"✅ ACE proactive greeting sent via on_enter hook (took {greeting_time:.1f}ms)"
                )

                # Log greeting as transcript
                self.latency_metrics["transcripts"].append(
                    {"timestamp": time.time(), "text": greeting, "role": "assistant"}
                )
                ...
                for msg in reversed(ctx.messages[-5:]):
                    if hasattr(msg, "role") and msg.role == "assistant":
                        ...
                        if isinstance(content, str) and content:
                            ...
                            existing = [
                                t
                                for t in self.latency_metrics["transcripts"]
                                if t["text"] == content[:50]
    ...
    def _on_metrics_collected(self, ev):
        ...
        for metric in ev.metrics:
            ...
            if speech_id and speech_id not in self._metrics_by_speech_id:
                ...
                self._metrics_by_speech_id[speech_id]["rag_duration"] = (
                    self._pending_rag_duration
                )
                self._pending_rag_duration = 0
            ...
            if isinstance(metric, metrics.EOUMetrics):
                if speech_id:
                    ...
                    logger.info(
                        f"⏱️  EOU Metrics (speech_id={speech_id}): "
            ...
            # Handle LLM metrics
                ...
                if speech_id:
                    ...
                    logger.info(
                        f"⏱️  LLM Metrics (speech_id={speech_id}): "
            ...
            # Handle TTS metrics
                ...
                if speech_id:
                    ...
                    logger.info(
                        f"⏱️  TTS Metrics (speech_id={speech_id}): "
            ...
            # Handle STT metrics
                ...
                if speech_id:
                    ...
                    logger.info(
                        f"⏱️  STT Metrics (speech_id={speech_id}): "
                        f"duration={metric.duration * 1000:.1f}ms, "
                        f"audio_duration={metric.audio_duration:.2f}s, "
    ...
    def _check_complete_turn(self, speech_id):
        ...
        if eou and llm and tts:
            total_latency_ms = (
                eou.end_of_utterance_delay * 1000 + llm.ttft * 1000 + tts.ttfb * 1000
            )
            ...
            turn_summary = {
                ...
                "on_user_turn_completed_delay_ms": eou.on_user_turn_completed_delay
                * 1000,
            ...
            logger.info(
                f"   🧠 LLM TTFT: {llm.ttft * 1000:.1f}ms | Total: {llm.duration * 1000:.1f}ms | {llm.completion_tokens} tokens"
            )
            logger.info(
                f"   🔊 TTS TTFB: {tts.ttfb * 1000:.1f}ms | Total: {tts.duration * 1000:.1f}ms"
            )
    ...
    async def on_user_turn_completed(
        ...
        logger.info(
            "🔔 HOOK FIRED: on_user_turn_completed"
        ...
        logger.debug(
            f"🔍 Raw content type: {type(content)}, value: {str(content)[:100]}"
        )
        ...
        if user_text:
            self.latency_metrics["transcripts"].append(
                {"timestamp": time.time(), "text": user_text, "role": "user"}
            )
            logger.info(f"📝 USER TRANSCRIPT: {user_text}")
        else:
            logger.error(
                f"❌ FAILED TO EXTRACT USER TEXT - content type: {type(content)}, value: {repr(content)[:200]}"
            )
        ...
        if any(keyword in user_text for keyword in self.IPC_KEYWORDS):
            try:
                logger.debug("RAG: IPC keywords detected, performing search...")
                ...
                if rag_content and "couldn't find" not in rag_content.lower():
                    ...
                    logger.debug(
                        f"RAG: No relevant knowledge found (took {rag_time:.1f}ms)"
                    )
            except Exception as e:
...
async def entrypoint(ctx: JobContext):
    ...
    if ctx.room.metadata:
        try:
            ...
            logger.warning("Ignoring invalid room metadata: %s", exc)
    ...
    try:
        ...
        session = AgentSession(
            ...
            tts=openai.TTS(
                model=config.tts_model, voice=config.tts_voice, speed=config.tts_speed
            ),
            vad=optimized_vad,
        ...
        logger.info(
            "Agent session initialized with latency-optimized components (fast VAD, preemptive generation)"
        )
        ...
        logger.info(
            "✅ Metrics collection enabled - will track EOU, LLM, TTS, and STT metrics"
        )
        ...
        if HAS_BACKGROUND_AUDIO:
            try:
                ...
                logger.info(
                    "Thinking audio enabled - will play during RAG/LLM processing"
                )
            except Exception as e:
                logger.warning(f"Could not enable thinking audio: {e}")
        else:
            logger.info(
                "Thinking audio not available (BackgroundAudioPlayer not in this LiveKit version)"
            )
        ...
        logger.info(
            f"✅ Agent session started successfully (took {session_start_duration:.1f}ms)"
        )
        ...
        try:
            ...
            if (
                hasattr(voice_agent, "transcription_node")
                and voice_agent.transcription_node
            ):
                ...
                def on_transcription(transcription):
                    ...
                    try:
                        if (
                            hasattr(transcription, "segments")
                            ...
                            for segment in transcription.segments:
                                ...
                                if text and len(text) > 2:
                                    ...
                                    existing = [
                                        t
                                        for t in voice_agent.latency_metrics[
                                            "transcripts"
                                        ]
                                        if t["text"] == text[:50]
                                        and abs(t["timestamp"] - time.time()) < 2
                                    ]
                                    if not existing:
                                        voice_agent.latency_metrics[
                                            "transcripts"
                                        ].append(
                                            {
                                                "timestamp": time.time(),
                ...
                if hasattr(voice_agent.transcription_node, "on"):
                    voice_agent.transcription_node.on(
                        "transcription_updated", on_transcription
                    )
                    logger.info("✅ Transcription event handler registered")
                else:
                    logger.warning(
                        "⚠️  Transcription node doesn't support event subscription"
                    )
            else:
                logger.warning(
                    "⚠️  Transcription node not available - using hook-based capture"
                )
        except Exception as e:
            logger.error(
                f"❌ Failed to set up transcription capture: {e}", exc_info=True
            )
...
if __name__ == "__main__":
    ...
    # Load local development values when present. Production deployments may
    # supply the same values directly through their environment.
    if env_path.exists():
        load_dotenv(env_path)
        logger.info(f"Loaded environment from: {env_path}")
    else:
        logger.info(
            f"No local environment file at {env_path}; using process environment"
        )
