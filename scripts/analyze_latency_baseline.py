...
def print_statistics_table(stats: Dict, label: str, unit: str = "ms"):
    ...
    print(
        f"    P90: {stats['p90']:.1f}{unit}  P95: {stats['p95']:.1f}{unit}  P99: {stats['p99']:.1f}{unit}"
    )
...
def print_baseline_summary(metrics_data: Dict):
    ...
    if metrics_data["turns"]:
        ...
        for i, turn in enumerate(metrics_data["turns"][-10:], 1):
            ...
            if "llm_ttft" in turn:
                print(
                    f"    LLM-TTFT: {turn['llm_ttft']:.1f}ms | LLM-Total: {turn.get('llm_total', 'N/A'):.1f}ms"
                )
            if "tts_ttfb" in turn:
                print(
                    f"    TTS-TTFB: {turn['tts_ttfb']:.1f}ms | TTS-Total: {turn.get('tts_total', 'N/A'):.1f}ms"
                )
...
def main():
    ...
    if not metrics_data["total_latencies"]:
        ...
        print(
            f"{Colors.YELLOW}   Make sure the agent is running and generating metrics.{Colors.END}"
        )
        sys.exit(1)
