...
class AgentOrchestrator:
    ...
    def is_agent_running(self):
        ...
        result = subprocess.run(
            ["pgrep", "-f", "python.*main.py.*dev"], capture_output=True, text=True
        )
    ...
    def stop_agent(self):
        ...
        subprocess.run(["pkill", "-f", "python main.py dev"], capture_output=True)
        print("🛑 Agent stopped")
    ...
    def agent_in_room(self, room_name):
        ...
        return (
            f"Agent starting for room: {room_name}" in log_content
            or f"room: {room_name}" in log_content.lower()
        )
    ...
    async def dispatch_to_room(self, room_name):
        ...
        try:
            ...
            request = api.CreateAgentDispatchRequest(
                agent_name=self.agent_name, room=room_name
            )
...
async def main():
    ...
    parser.add_argument(
        "--dispatch-only", action="store_true", help="Dispatch once and exit"
    )
